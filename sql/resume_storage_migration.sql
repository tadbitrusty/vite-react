-- Resume Storage and Intelligence Pipeline Migration
-- This creates the complete data collection infrastructure for market intelligence
...
-- Admin access for analytics (whitelist check)
...
-- Storage buckets setup (run this in Supabase dashboard)
-- CREATE BUCKET IF NOT EXISTS 'resume-files';
-- CREATE BUCKET IF NOT EXISTS 'resume-images';
-- CREATE BUCKET IF NOT EXISTS 'generated-pdfs';

-- Storage policies (run these in Supabase dashboard)
-- Allow authenticated users to upload files
-- CREATE POLICY "Allow authenticated uploads" ON storage.objects
--   FOR INSERT WITH CHECK (bucket_id IN ('resume-files', 'resume-images', 'generated-pdfs'));

-- Allow users to access their own files
-- CREATE POLICY "Allow user file access" ON storage.objects
--   FOR SELECT USING (bucket_id IN ('resume-files', 'resume-images', 'generated-pdfs'));

COMMENT ON TABLE resume_files IS 'Stores original resume file uploads for the intelligence platform';
COMMENT ON TABLE resume_processing_jobs IS 'Tracks AI processing pipeline status and metadata';
COMMENT ON TABLE resume_intelligence IS 'The core data goldmine - extracted intelligence for market analysis';
COMMENT ON TABLE email_deliveries IS 'Email delivery tracking and analytics';
COMMENT ON TABLE market_intelligence_analytics IS 'Aggregated business intelligence and market insights';