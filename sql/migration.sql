...
-- This script creates all necessary tables for the Resume Vita application
...
-- Insert predefined whitelist entries (the ones that were in memory)
...
-- Note: In production, you would create more restrictive policies based on your auth system
CREATE POLICY "Enable all operations for service role" ON user_sessions FOR ALL USING (auth.role() = 'service_role');
CREATE POLICY "Enable all operations for service role" ON whitelist_entries FOR ALL USING (auth.role() = 'service_role');
CREATE POLICY "Enable all operations for service role" ON abuse_patterns FOR ALL USING (auth.role() = 'service_role');
CREATE POLICY "Enable all operations for service role" ON admin_sessions FOR ALL USING (auth.role() = 'service_role');

-- Grant necessary permissions to anon and authenticated roles if needed
...
DO $$
BEGIN
    RAISE NOTICE 'Resume Vita database schema migration completed successfully!';
    RAISE NOTICE 'Tables created: user_sessions, whitelist_entries, abuse_patterns, admin_sessions';
    RAISE NOTICE 'Indexes created for optimal performance';
    RAISE NOTICE 'Predefined whitelist entries inserted';
